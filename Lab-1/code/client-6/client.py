...
class Client(object):
    ...
    def pushedScores(self, event, score):
	#updated scores from update server is immediately pushed on to regsitered client.
	print "*****SERVER PUSH UPDATE*******"
	tokens = score.split(":")	
	print event + " Gauls:" + str(tokens[0]) + " Romans:"+str(tokens[1])
	print "*****UPDATE ENDS******"
	return
...
def main():
    ...
    if sys.argv[1] == "1":
        server.registerClient(client, 'curling') #registering a client onto server for receiving SERVER PUSH update.
