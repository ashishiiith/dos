#How to run this script:"python server1.py <argument>". Argument is either 1(push) or 2(pull).
#Funtionality: This code handles the front-end server with ID=1. It also has a cache implemented in it.
...
class serverPush(object):
	...
	try:
	    ...
	    if push == 1:
	        ...
	        try:
                    server=Pyro4.core.Proxy("PYRONAME:example1.server2")
...
class scoreRead(object):
	...
	failed_server=3-idNum #other server
	...
	try:
	    ...
	    print "stale data is removed from cache.."
	...
	try:
		...
		print "***********************************"
                l = database.getScore(eventType) #in case of cache miss, access database
...
def main():
    ...
    print "Server1 Ready..."
    #'''starting the cache consistency protocol'''
